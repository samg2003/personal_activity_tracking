...
- Currently everything gets deleted when i delete curr data or if i import. Ensure deleting or even importing, exporting, ensures default categories, exercises, muscle groups, etc. is not impacted. For import, There could be additional, but not less, except for categories which has restore default categories option.
